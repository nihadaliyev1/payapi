...
})({"eVXrx":[function(require,module,exports) {
...
module.bundle.HMR_BUNDLE_ID = "60e314a54d7566d7";
"use strict";
...
contactForm.addEventListener("submit", (e)=>{
    ...
    allInputs.forEach((el)=>{
        ...
        if (!el.value.trim()) {
            ...
            if (!formErrors.includes(`${el.dataset.name} empty error`)) formErrors.push(`${el.dataset.name} empty error`);
        } else {
            closestContainer.classList.remove("input-error");
            formErrors = formErrors.filter((err)=>!(`${el.dataset.name} empty error` === err));
        }
    });
    // Email Validation
    if (contactInputEmail.value && !validateEmail(contactInputEmail.value)) {
        contactInputEmail.closest(".contactus__input-wrapper").classList.add("input--email-error");
        if (!formErrors.includes(`email invalid error`)) formErrors.push(`email invalid error`);
    } else {
        contactInputEmail.closest(".contactus__input-wrapper").classList.remove("input--email-error");
...
},{}]},["eVXrx","fkrlx"], "fkrlx", "parcelRequire8a8b")

//# sourceMappingURL=contact.4d7566d7.js.map
